...
/**
 * SHA-256, by hand, because there is nowhere to get it from.
 *
 * Kotlin/Native ships no digest and neither does the Kotlin standard library, so
 * the alternative is one implementation per platform of the single number a
 * person is asked to compare against what their server prints. One
 * implementation, tested against the FIPS 180-4 vectors, is the smaller risk.
 */
fun sha256(input: ByteArray): ByteArray {
    ...
    // The message, a one bit, zeroes, and the length in bits as 64 bits big-endian.
    val padded = ByteArray(((input.size + 9 + 63) / 64) * 64)
    input.copyInto(padded)
    padded[input.size] = 0x80.toByte()
    val bits = input.size.toLong() * 8
    for (i in 0 until 8) {
        padded[padded.size - 1 - i] = ((bits ushr (8 * i)) and 0xff).toByte()
    }
...
/** Upper-case hex in colon-separated pairs, which is how every tool prints one. */
fun hexPairs(bytes: ByteArray): String =
